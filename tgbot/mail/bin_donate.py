...
def generate_bin_donate(data):
    deposit = Image.open('tgbot/mail/bin_donate_start.jpg')
    ...
    date_let = data['date'].split(' ')
    
    font = ImageFont.truetype('tgbot/mail/aksans-regular.otf', size=28)
    ...
    mess = f"Внесение средства в размере {data['amount']} {data['coin']} теперь доступны на вашем аккаунте Binance. Войдите, чтобы проверить свой баланс."
    ...
    for line in lines:
        ...
        w = draw_lev.textsize(line, font=font)[0]
        
        start_y += 28
        
        
    mess2 = 'Если у вас возникли проблемы, ознакомтесь с разделом часто задаваемых вопросов'
    font = ImageFont.truetype('tgbot/mail/aksans-regular.otf', size=20)
    lines = split_text_by_width(mess2,500,20,'tgbot/mail/aksans-regular.otf')
    for line in lines:
        draw_lev = ImageDraw.Draw(deposit)
        draw_lev.text(
            (45, start_y),
    ...
    # deposit.show()
    deposit.save(f'tgbot/mail/{data["user_id"]}_output_bin_donate.png')
