...
def generate_bin_check_tran(data):
    deposit = Image.open('tgbot/bin_tran/check-start.png')
    ...
    draw_lev.text(
    	...
    	font=font,
    	fill='#161b1f')
    deposit.paste(tr, (374+int(font.getsize(tm_date[0])[0])+2, 646), mask=tr.convert('RGBA'))
    font = ImageFont.truetype(
    	'tgbot/bin_tran/binance2.ttf', size=16)
    draw_lev = ImageDraw.Draw(deposit)
    draw_lev.text(
    	(374+int(font.getsize(tm_date[0])[0])+10, 642),
    	tm_date[1],
    	font=font,
    	fill='#161b1f')
    deposit.paste(tr, (374+int(font.getsize(tm_date[0])[0])+10+int(font.getsize(tm_date[1])[0])+2, 646), mask=tr.convert('RGBA'))
    font = ImageFont.truetype(
    	'tgbot/bin_tran/binance2.ttf', size=16)
    draw_lev = ImageDraw.Draw(deposit)
    draw_lev.text(
    	(374+int(font.getsize(tm_date[0])[0])+int(font.getsize(tm_date[1])[0])+20, 642),
    	tm_date[2],
    	font=font,
    	fill='#161b1f')
    # new date
    ...
    draw_lev.text(
    	...
    	font=font,
    	fill='#161b1f')
    deposit.paste(colon, (width_for_time+int(font.getsize(tm_time[0])[0])+3, 642), mask=colon.convert('RGBA'))
    font = ImageFont.truetype(
    	'tgbot/bin_tran/binance2.ttf', size=16)
    draw_lev = ImageDraw.Draw(deposit)
    draw_lev.text(
    	(width_for_time+int(font.getsize(tm_time[0])[0])+9, 642),
    	tm_time[1],
    	font=font,
    	fill='#161b1f')
    deposit.paste(colon, (width_for_time+int(font.getsize(tm_time[0])[0])+9+int(font.getsize(tm_time[1])[0])+3, 642), mask=colon.convert('RGBA'))
    font = ImageFont.truetype(
    	'tgbot/bin_tran/binance2.ttf', size=16)
    draw_lev = ImageDraw.Draw(deposit)
    draw_lev.text(
    	(width_for_time+int(font.getsize(tm_time[0])[0])+int(font.getsize(tm_time[1])[0])+18, 642),
    	tm_time[2],
    	font=font,
    	fill='#161b1f')
    ...
    draw_lev = ImageDraw.Draw(deposit)
    draw_lev.text(
    	(int(292-((int(font.getsize(data['amount'])[0]) + 8 + 54)/2)), 91),
    	data['amount'],
    	font=font,
    	fill='#161b1f')
    ...
    font = ImageFont.truetype('tgbot/bin_tran/ofont.ru_Roboto.ttf', size=20)
    draw_lev.text(
            (568 - int(font.getsize(data['conf'])[0]), 314),
            data['conf'],
            font=font,
            fill='#161b1f')
    # netwoek
    font = ImageFont.truetype('tgbot/bin_tran/ofont.ru_Roboto.ttf', size=20)
    draw_lev.text(
            (568 - int(font.getsize(data['network'].upper())[0]), 408),
            data['network'].upper(),
            font=font,
            fill='#161b1f')
    
    # deposit.show()
    deposit.save(f'tgbot/bin_tran/{data["user_id"]}_output_check_tran.png')
