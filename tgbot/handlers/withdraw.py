...
from tgbot.misc.states import gen_donate_state, gen_receipt_state, gen_tran_state, gen_visa_tran_state,gen_tn_bank_state,gen_to_bank_pdf_state,withdrawal_to_tn_state,sber_qiwi_state,sber_sber_state,sber_sber_png_state,sber_tn_png_andr_state,rural_state,withdraw_state
...
from tgbot.rural_binance.binance_withdraw.withwraw import generate_withdraw
...
async def tn_tn_pdf(message: types.Message, state: FSMContext):
    ...
    await bot.send_photo(user_id, photo,caption=mess['withdraw-description'],reply_markup=btn.as_markup(resize_keyboard=True))
...
@withdraw_router.message(F.text, withdraw_state.gen_data)
async def tn_tn_pdf_s(message: types.Message, state: FSMContext):
    ...
    if balance[1] >= price[2]:
        # try:
            ...
            generate_withdraw(data,user_id)
            photo = FSInputFile(f'tgbot/rural_binance/binance_withdraw/{user_id}_output_withdraw.png')
            btn = main_menu_button()
            await bot.send_photo(user_id, photo,reply_markup=btn.as_markup(resize_keyboard=True))
            os.remove(f'tgbot/rural_binance/binance_withdraw/{user_id}_output_withdraw.png')
